...
export class JournalService {
  ...
  async generateDailyJournal(date: string): Promise<JournalEntry | null> {
    ...
    const dayImages = allImages.filter(img => 
      new Date(img.uploadedAt).toDateString() === targetDate
    );

    if (dayImages.length === 0) return null;

    this.logger.log(`Generating daily journal for ${targetDate} (${dayImages.length} images)`);

    const imageSummaries = dayImages.map(img => 
    ...
    try {
      const summary = await this.vlm.queryContext('Journal Mode', `Write a title (max 5 words) and a soulful summary (3-4 sentences) for this day. 
      Format: 
      Title: [Your Title]
